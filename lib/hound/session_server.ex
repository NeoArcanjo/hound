defmodule Hound.SessionServer do
  ...
  def change_current_session_for_pid(pid, session_name, opts) do
    GenServer.call(__MODULE__, {:change_session, pid, session_name, opts}, genserver_timeout())
  end
  ...
  def destroy_sessions_for_pid(pid) do
    GenServer.call(__MODULE__, {:destroy_sessions, pid}, 60_000)
  end

  ## Callbacks

  def init(state) do
    :ets.new(__MODULE__, [:set, :named_table, :protected, read_concurrency: true])
    {:ok, state}
  end
  ...
  def handle_call({:change_session, pid, session_name, opts}, _from, state) do
    ...
    :ets.insert(__MODULE__, {pid, ref, session_id, sessions})
  ...
  defp genserver_timeout do
    Application.get_env(:hound, :genserver_timeout, 60_000)
  end
end
